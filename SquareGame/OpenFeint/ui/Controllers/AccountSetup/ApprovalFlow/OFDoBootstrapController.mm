...
- (void)_findUsersSucceeded:(OFPaginatedSeries*)resources
{
	...
	if (numUsers == 0)
	{
		[OpenFeint doBootstrapAsNewUserOnSuccess:OFDelegate(self, @selector(_bootstrapSucceded)) onFailure:OFDelegate(self, @selector(_bootstrapFailed))];
	}
	else if (numUsers == 1)
	{
		[OpenFeint doBootstrapAsUserId:[(OFUser*)[resources objectAtIndex:0] resourceId] onSuccess:OFDelegate(self, @selector(_bootstrapSucceded)) onFailure:OFDelegate(self, @selector(_bootstrapFailed))];
	}
	else
	{
